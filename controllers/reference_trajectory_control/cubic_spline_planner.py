...
class CubicSpline3D:
    ...
    Examples
    --------
    You can interpolate a 3D data points.
    ...
    def calc_curvature(self, s):
        ...
        dy = self.sy.calc_first_derivative(s)
        ddy = self.sy.calc_second_derivative(s)
        dz = self.sy.calc_first_derivative(s)
        ddz = self.sy.calc_second_derivative(s)
        k = (ddy * dx - ddx * dy) / ((dx ** 2 + dy ** 2)**(3 / 2))
        return k
