...
class QtmWrapper(Thread):
    ...
    def __init__(self, qtm_ip, cf_body_names):
        ...
        self.qtm_ip = qtm_ip
        self.cf_body_names = cf_body_names
        self.cf_pose = None
        self.ob_pose = None
    ...
    async def _connect(self):
        ...
        for body_name in self.cf_body_names:
            if body_name in self.bodyToIdx:
                print("Body '" + body_name + "' found in QTM 6DOF bodies.")
            else:
                print("Body '" + body_name + "' not found in QTM 6DOF bodies!")
                print("Aborting...")
                self._stay_open = False
    ...
    def _on_packet(self, packet):
        ...
        cf_6d = component_6d[self.bodyToIdx[self.cf_body_names[0]]]
        # Store in temp until validity is checked
        _cf_pose = Pose.from_qtm_6d(cf_6d)
        # Check validity
        if _cf_pose.is_valid():
            # Update global var for pose
            self.cf_pose = _cf_pose
            #print(f'mocap pos: x: {cf_pose.x}, y: {cf_pose.y}, z: {cf_pose.z}')
            # Stream full pose to Crazyflie
            if self.on_cf_pose:
                self.on_cf_pose([self.cf_pose.x, self.cf_pose.y, self.cf_pose.z, self.cf_pose.rotmatrix])
                self.cf_trackingLoss = 0
        else:
            self.cf_trackingLoss += 1
        
        # Get 6DOF data for the obstacle
        ob_6d = component_6d[self.bodyToIdx[self.cf_body_names[1]]]
        _ob_pose = Pose.from_qtm_6d(ob_6d)
        if _ob_pose.is_valid():
            self.ob_pose = _ob_pose
    
    def getpose(self):
        return self.cf_pose, self.ob_pose
