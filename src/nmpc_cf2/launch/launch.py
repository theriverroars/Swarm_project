...
def parse_yaml(context):
    ...
    server_yaml = os.path.join(
        get_package_share_directory("nmpc_cf2"), "config", "server.yaml"
    )
    ...
    singlemarker2fullpose_params = dict()
    singlemarker2fullpose_params["crazyflies"] = []
    ...
    for key, value in crazyflies["robots"].items():
        ...
        if value["enabled"] and type["motion_capture"]["enabled"]:
            ...
            singlemarker2fullpose_params["crazyflies"].append(key)

    # copy relevent settings to server params
    server_params[1]["poses_qos_deadline"] = motion_capture_params["topics"]["poses"][
        "qos"
    ]["deadline"]
    singlemarker2fullpose_params["mocap_deadline"] = motion_capture_params["topics"][
        "poses"
    ]["qos"]["deadline"]
    ...
    return [
        ...
        Node(
            package="nmpc_cf2",
            executable="singlemarker2fullpose",
            condition=IfCondition(
                PythonExpression(["'", LaunchConfiguration("mocap"), "' == 'True'"])
            ),
            name="singlemarker2fullpose",
            output="screen",
            parameters=[singlemarker2fullpose_params],
        ),
    ]
...
def generate_launch_description():
    default_crazyflies_yaml_path = os.path.join(
        get_package_share_directory("nmpc_cf2"), "config", "crazyflies.yaml"
    )

    default_motion_capture_yaml_path = os.path.join(
        get_package_share_directory("nmpc_cf2"), "config", "motion_capture.yaml"
    )

    default_rviz_config_path = os.path.join(
        get_package_share_directory("nmpc_cf2"), "config", "config.rviz"
    )

    telop_yaml_path = os.path.join(
        get_package_share_directory("nmpc_cf2"), "config", "teleop.yaml"
    )
    ...
    return LaunchDescription(
        [
            ...
            DeclareLaunchArgument(
                "rviz_config_file", default_value=default_rviz_config_path
            ),
            ...
            Node(
                ...
                parameters=[
                    PythonExpression(
                        [
                            "'" + telop_yaml_path + "' if '",
                            LaunchConfiguration("teleop_yaml_file"),
                            "' == '' else '",
                            LaunchConfiguration("teleop_yaml_file"),
                            "'",
                        ]
