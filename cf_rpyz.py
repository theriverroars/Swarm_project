""" Program for postion tracking with cf from a csv file"""
import time
import pandas as pd
import cflib.crtp
...
def log_stabilizer_callback(timestamp, data, logconf):
    global INPUTS
    INPUTS['stab_timestamp'].append(timestamp)
    INPUTS['stab_thrust'].append(data['stabilizer.thrust'])
...
if __name__ == '__main__':
    ...
    if x == 'y' or x == 'Y':
        ...
        DF = DF.transpose()
        DF.to_csv('/home/rajpal/github_dat/Drones-C3BF-hardware/data_log/dynamics_dataset {}.csv'.format(time_string))
