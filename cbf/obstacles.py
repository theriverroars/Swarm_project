...
class Ellipse3D(Obstacle3DBase):
    ...
    """
    def __init__(self, a: float, b: float, c: float, center: Vector3 = Vector3(0, 0), theta: float=0, buffer: float=0):
        """
    ...
    def __repr__(self):
        return f"{type(self).__name__}(a = {self.a}, b = {self.b}, c = {self.c}, center = {self.center}, theta = {self.theta}, buffer = {self.buffer}, buffer_applied: {self.BUFFER_FLAG} )\n"
    ...
    def update_state(self, xyz: Point3, theta: float, v: Vector3):
    ...
    def from_bounding_box(cls, bbox = BoundingBox(), buffer = 0.5) -> Ellipse3D:
        ...
        c = bbox.extent.z
        center = Vector3(bbox.location.x, bbox.location.y, bbox.location.z)
        theta = bbox.rotation.roll
        theta = bbox.rotation.pitch
        theta = bbox.rotation.yaw
        return cls(a, b, c, center, theta, buffer)
...
class CollisionCone3D(Obstacle3DBase):
    ...
    def __init__(self, 
                 a: float = 0.0, 
                 s: matrix = matrix(0, (7,1)), 
                 s_obs: matrix = matrix(0, (7,1)),
                 buffer: float=1.50):
        """
        ...
        self.s_vx = s[6]*np.cos(s[2])
        self.s_vy = s[6]*np.sin(s[2])
        self.s_vz = s[6]*np.sin(s[2])
        self.s_obs_vx = s_obs[6]*np.cos(s_obs[2])
        self.s_obs_vy = s_obs[6]*np.sin(s_obs[2])
        self.s_obs_vz = s_obs[6]*np.sin(s_obs[2])
    ...
    @classmethod
    def from_bounding_box(cls, s: matrix = matrix(0.0, (4,1)), bbox = BoundingBox(), buffer = 0.5) -> CollisionCone3D:
        if not isinstance(bbox, BoundingBox):
            raise TypeError("Expected an object of type cbf.obstacles.BoundingBox as an input to fromBoundingBox() method, but got ", type(bbox).__name__)
