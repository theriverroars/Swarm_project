...
try:
    from cbf.obstacles import ObstacleList3D, ObstacleList2D
...
class DDM_FC_CBF3D():
    """
    The Dynamic Drone Model - Force Controlled - Control Barrier Function.
    ...
    def __init__(self, alpha = 1.0):
        self.obstacle_list3d = ObstacleList3D()
        self.__alpha = alpha
    ...
    def solve_cbf(self, u_ref):
        ...
        """
        m = len(self.obstacle_list3d) # No. of non-linear constraints
        n = 4 # dimension of x0 => u0
        u_ref = matrix(u_ref)

        if len(self.obstacle_list3d) < 1:
        ...
        def F(x = None, z=None):
            ...
            # G => Gradient, Gh -> (m,3)
            Gh = matrix([ [self.obstacle_list3d.dx(self.__p)], [self.obstacle_list3d.dy(self.__p)], [self.obstacle_list3d.dz(self.__p)], [self.obstacle_list3d.dphi(self.__p)], [self.obstacle_list3d.dtheta(self.__p)], [self.obstacle_list3d.dpsi(self.__p)] ])
            
            Lxg_h = Gh * gc

            f[1:] = -( Lxg_h * x + ( self.__alpha * self.obstacle_list3d.f(self.__p) ) )
            Df[1:, :] = -Lxg_h
